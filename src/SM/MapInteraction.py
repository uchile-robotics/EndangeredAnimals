...
class Example(smach.State):
    ...
    def execute(self,userdata): # Userdata es informacion que se puede mover entre estados
        ...
        userdata.animal_region=self.ZonesNames[self.last_zone]
...
def getInstance(robot):
    sm =smach.StateMachine(outcomes=['succeeded','aborted'],output_keys=['animal_region'])
    ...
    with sm:
        ...
        smach.StateMachine.add('INSTRUCTIONS',Speak(robot,"Ahora porfavor realiza lo siguiente"),
            transitions={
                'succeeded':'EXAMPLE1'
            }
        )
